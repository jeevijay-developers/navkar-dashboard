...
export function AppProvider({ children }) {
  ...
  const handleAddProduct = async (productData, imageFile = null) => {
    try {
      ...
      let newProduct;
      if (imageFile) {
        newProduct = await serverAPI.createProductWithImage(
          productData,
          imageFile
        );
  ...
  const handleUpdateProduct = async (id, productData, imageFile = null) => {
    try {
      ...
      let updatedProduct;
      if (imageFile) {
        updatedProduct = await serverAPI.updateProductWithImage(
          id,
          productData,
          imageFile
        );
  ...
  const handleBulkUpload = async (file) => {
    try {
      ...
      toast.success(
        `Successfully uploaded ${result.successCount || 0} products!`
      );
      return result;
    } catch (err) {
      setError(err.message);
  ...
  const handleResendWhatsApp = async (quotationId, options) => {
    try {
      setLoading(true);
      setError(null);
      const result = await serverAPI.resendQuotationWhatsApp(
        quotationId,
        options
      );
      await fetchQuotations(); // Refresh quotations list
